...
class Thermostat_MH52D extends ZwaveDevice {
  async onNodeInit() {
      ...
      setParserV2: thermostatMode => {
        ...
        if (this.getCapabilityValue('thermostat_mode_hvac') !== thermostatMode) {
          const thermostatModeObj = {
            mode: thermostatMode,
            mode_name: thermostatMode,
          };
          this.triggerThermostatModeChanged.trigger(this, thermostatModeObj, null).catch(this.error);
          this.triggerThermostatModeChangedTo.trigger(this, null, thermostatModeObj).catch(this.error);
        }
    ...
    this.registerCapability('fan_speed_MH52D', 'THERMOSTAT_FAN_MODE', {
      getOpts: {
        getOnStart: true,
      },
      ...
      setParserV2: fanMode => {
        ...
        if (this.getCapabilityValue('fan_speed_MH52D') !== fanMode) {
          const fanModeObj = {
            mode: fanMode,
            mode_name: fanMode,
          };
          this.triggerFanModeChangedTo.trigger(this, null, fanModeObj).catch(this.error);
        }
        return {
          Properties1: {
            Off: false,
    ...
    this.triggerThermostatModeChangedTo = this.homey.flow.getDeviceTriggerCard('thermostat_mode_MH52D_changed_to')
      .registerRunListener((args, state) => Promise.resolve(args.mode === state.mode));

    // fan_speed_MH52D_changed_to
    this.triggerFanModeChangedTo = this.homey.flow.getDeviceTriggerCard('fan_speed_MH52D_changed_to')
      .registerRunListener((args, state) => Promise.resolve(args.mode === state.mode));

    // Register actions for flows thermostat_change_mode
    this._actionThermostatChangeMode = this.homey.flow.getActionCard('change_thermostat_mode_MH52D')
      .registerRunListener((args, state) => {
        ...
        return args.device.triggerCapabilityListener('thermostat_mode_hvac', thermostatMode, {});
      });

    // Register actions for flows
    this._actionThermostatChangeSetpoint = this.homey.flow.getActionCard('change_thermostat_mode_setpoint_MH52D')
      .registerRunListener(this._actionThermostatChangeSetpointRunListener.bind(this));

    // Register actions for flows change_fan_speed_MH52D
    this._actionFanChangeMode = this.homey.flow.getActionCard('change_fan_speed_MH52D')
      .registerRunListener((args, state) => {
        const fanMode = args.mode;
        this.log('FlowCardAction triggered for ', args.device.getName(), 'to change Fan mode to', fanMode);

        // Trigger the thermostat mode setParser
        return args.device.triggerCapabilityListener('fan_speed_MH52D', fanMode, {});
      });
